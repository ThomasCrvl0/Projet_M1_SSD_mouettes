...
\begin{center}
$\prob_\theta(Z = j | X=x ) = \frac{\displaystyle\alpha_j \times \gamma_{\mu_j, v_j}(x)}{\displaystyle\sum_{i=1}^J\alpha_i \gamma_{\mu_i, v_i}(x)}$, pour $i\in \{1,\cdots,J\}$
\end{center}
\end{prop}
...
\begin{center} $h_\theta(x,j) := \alpha_j\gamma_{\mu_j, v_j}(x)$ \end{center}
\end{prop}
...
En effet, nous avons
\begin{align*}
\kappa_{\theta_{k}, \theta_k}-\kappa_{\theta_{k+1}, \theta_k} &= \sum_{i=1}^n\sum_{j=1}^J ln(\prob_{\theta_k}(Z = j|X=X_i))\prob_{\theta_k}(Z = j|X = X_i) - \sum_{i=1}^n \sum_{j=1}^J ln(\prob_{\theta_{k+1}}(Z = j|X=X_i))\prob_{\theta_k}(Z = j|X = X_i) \\
