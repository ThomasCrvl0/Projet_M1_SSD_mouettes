...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\REQUIRE{$K \in \mathbb{N}$, $\widehat{\theta_0} \in \Theta$, un jeu de données $x_1 \dots x_n$;}
		\ENSURE
		\STATE {$k:=1$;}
		\FOR {$k $ de $1$ à $K$}
		...
		\STATE {$\text{\textbf{ETAPE M : }} \widehat{\theta}_k = \underset{\theta}{\text{argmax}} \hspace{1.5mm} Q(\theta;\widehat{\theta}_{k-1})$;}
...
Nous devons maintenant déterminer les paramètres initiaux. \newline
Notre première méthode sera basée sur des heuristiques utilisant la fonction de densité de l'échantillon. Commençons par tracer cette dernière :
\begin{center}
\includegraphics[scale=0.8]{dens_1.pdf}
\end{center}

Nous distinguons ici nettement le mélange des deux gaussiennes; un premier pic est présent autour de la valeur $\mu_{1}=40$; et un second autour d'environ et $\mu_{2}=300$. Ces pics étant bien distinct, ils nous donnent de bonnes approximations quant aux valeurs initiales pour les moyennes. Ainsi, $\mu_{1_{init}} = 40$ et $\mu_{2_{init}} = 300$\newline
Concernant l'écart type, nous utiliserons l'heuristique suivante qui est effroyablement approximative : \newline
Nous savons que pour une variable aléatoire $X$ telle que $X\sim\mathcal{N}(\mu, \sigma^2)$, nous avons $\prob(X \in [\mu\pm\sigma]) \approxeq 0.7$. De ceci, nous pouvons extraire une estimation très approximative de l'écart-type.  \newline
Nous allons donc tenter d'englober environ $70\%$ des données autour de la moyenne. Nous considèrerons et le diagramme en bâton et la densité pour cela. 
\newline
Nous traçons donc ici des droites verticales; à $\mu_{1_{init}} \pm 30$ et à $\mu_{2_{init}} \pm 13	0$. Le choix de ces valeurs s'est effectué à tâtons, de manière toute à fait approximative.	 

\begin{center}
\includegraphics[scale=0.8]{dens_1bis.pdf}
\end{center}
Nous englobons une proportion raisonnable des données autour de chaque pics; $\sigma_{1_{init}} = 30$ et $\sigma_{2_{init}} = 130$ seront ainsi nos valeurs initiales pour les écarts-types.\newline
Concernant les proportions, nous les considérerons comme égales, de fait $\alpha_{1_{init}} = \alpha_{2_{init}} = 0.5$. \newline
Nous avons donc tous les éléments nécessaire. Nous commençons par créer le dataframe avec les paramètres initiaux :
...
1 European Goldfinch 0.2945556  37.36098   8.967584
2         Ring Ouzel 0.7054444 291.64564 127.615227
\end{verbatim}
...
Les proportions sont toutes deux très bien estimées, les erreurs sont de l'ordre de moins de $1\%$, ce qui est formidable.
La moyenne de la première espèce présente elle aussi une erreur d'estimation de l'ordre de $1\%$, celle de la seconde une erreur d'estimation de l'ordre de $3\%$, ce qui est très satisfaisant. Il en est de même pour les variances. \newline
Pour ce premier exemple d'un mélange de deux lois, nous obtenons de très bonne approximations.

% section 3
\section{Etude d'un mélange à trois lois}
