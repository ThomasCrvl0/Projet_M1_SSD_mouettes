...
Pour une taille d'échantillon n fixée, nous avons exécuté k fois l'algorithme EM pour des échantillons différents. À partir de cela, nous avons calculé les $3 \times 3 \times K$ erreurs absolues entre les paramètres estimés par l'algorithme EM et les paramètres théoriques. Elle prend en argument :
\begin{itemize}
...
Afin de se conformer aux hypothèses que nous avons précédemment mentionné, nous supposerons ici que les volumes des nids suivent une loi normale. Nous ferons une dernière hypothèse; nous supposerons le nombre de lois connu lors de l'observation. \newline
...
\begin{center}
$
\widehat{f_n}(x) = \displaystyle \frac{1}{nh} \sum_{i=1}^n K\Big(\frac{x - X_i}{h}\Big)
$
\end{center}
...
\item Une seconde solution est d'utiliser est d'utiliser les quantiles pour l'estimation des moyennes, et l'écart-type empirique pour les variances initiales. 
\end{itemize}

Une fois cela fait, nous pouvons exécuter notre algorithme et obtenir son estimation des paramètres. Nous pourrons dès lors émettre les conclusions appropriées.\newline
...
Cette section sera composée de deux sous-sections. Dans la première, nous déterminerons les paramètres initiaux via la première méthode décrite précedemment. Nous pourrons dès lors réaliser une première exécution de notre implémentation de l'algorithme EM.
...
Les proportions sont toutes deux très bien estimées, les erreurs sont de l'ordre de $1\%$.Il en est de même pour les moyennes, les erreurs d'estimations sont de l'ordre de $1\%$, , ce qui est plus que satisfaisante. Les variances sont de mêmes très bien estimées.
...
Pour l'initalisation des écarts-types, nous prendrons 
\begin{center}
...
\end{center}
est l'écart-type empirique; avec $J$ le nombre de mélanges et $n$ la taille de l'échantillon. \newline
...
Nous construisons le dataframe des paramètres initiaux : 
\begin{lstlisting}
...
Comme pour le cas précédent, comparons les avec les valeurs théoriques. Nous rappelons une fois de plus les paramètres théoriques :
\begin{verbatim}
         bird_names2 proportion_alpha mean_volume sd_volume
1 European Goldfinch         0.2878713       38.0       9.1
12         Ring Ouzel        0.7121287      298.6     125.1
