...
Pour l'implémentation de cet algorithme, nous nous sommes appuyés sur le pseudo-code suivant, insipré de la référence [3].
...
\section{Automatisation des paramètres initaux}

Dans le cas eventuel où l'utilisateur ne souhaite pas ou ne pas peut pas effectuer de descision, quant au choix des paramètres initaux de l'algorithme EM, on se propose de présenter trois fonctions automatisant ce choix. \\\\

On présente dans un premier temps les deux fonctions $param\_quantile1$ et $param\_quantile2$. Elles reposent sur une approche plus simplicite et rendent compte, pour le choix des paramètres $\alpha_j,~\forall j \in [1,...,J]$ (porportions des J espèces observées) et $v_j,~\forall j \in [1,..,J]$ (variance du volume des J espèces observées), des idées suivantes : 

Si l'échantillon est obtenu aléatoirement, sur une zone d'étude tirée aléatoirement, on pourrait supposer que la quantité de chaque espèce sur cette zone est approximativement la même. 
On choisira ainsi des proprtions égales pour le choix d'initialisation des paramètres $\alpha_j,~\forall j \in [1,...,J]$.

De même, il peut être vraisemblable de ce dire, pour des espèces semblables, que chaque espèce font des nids approximativement de même volume. Ainsi on pourra choisir comme écart-type, des écart-types égaux.\\
...
Comme décrit en debut de section, on choisi des proportions égales entre espèce. On a alors $\alpha_j$, les valeurs $\alpha_j = \frac{1}{J},~\forall j \in [1,..J]$.

Puisque que les espèces font (dans l'idée de cette fonction) des nids semblables, on va trier pour la suite notre échantillon. 

Pour le choix des moyennes le principe est le suivant : 
On découpe l'intervalle des observations en J+1 intervalles, chacune des coupures correspondra (suivant la parité de l'échantillon) à decouper l'intervalle de façon uniforme.
On prendra comme moyennes les J premières valeurs associées à ces coupures. Approximativement cela correspond à prendre les J premiers quantiles.
...
Quant aux choix des écart-types de chaque espèce, on choisira l'écat-type de la première espèce. \\
...
La distinction qui se fait entre cette fonction et celle précédente est le choix des moyennes pour chaque espèce. 
...
\begin{rem}
Même sous réserve d'avoir des proportions d'espèces proches, il n'est pas assuré que l'une des deux fonctions donne sans contestement une meilleure initialisation de l'algorithme EM.
On peut envisager dans ce cas là, de considérer les deux fonctions et tester leurs résultats en appliquant l'algorithme EM. Il s'agira donc d'évaluer et de comparer la log-vraisemblance de la variable $X$ en les valeurs de sortie de l'algorithme EM, avec pour choix de paramètres initaux, ceux des fonctions présentées. On retiendra comme initalisation, celle dont la log-vraisemblance de $X$ sera la plus grande. \\
\end{rem}

Afin de se donner le plus de chance, quant à la détermination des estimateurs de maximum de vraisemblance de $X$. On en visage la fonction d'initialisation $param\_kmeans$, en espérant en plus qu'elle soit plus efficace que les deux précédentes.
Par soucis de place, celle-ci sera donnée en Annexe.

Décrivons tout fois son code et son principe :
...
Il s'agit dans cette fonction de choisir une initialisation des paramètres initiaux dans un cadre plus générale que celui des deux fonctions précédente (en supposant égalité des coefficients $\alpha_j$ ou encore des $v_j$). \\

La démarche est la suivante : 
Etant donné une variable aléatoire X suivant une loi normale de paramètres quelqu'ils soient, la moyenne de variable X correspondra au maximum de ses valeurs. 
Dans ce cas, et si on dispose d'un échantillon "plus ou moins bien hétérogène" (quant aux paramètres des lois normales du mélange) il est possible de choisir comme initialisation des moyennes, les moyennes exactes des espèces. 
Dans le cas d'un mélange plus "homogène", il est possible de relever sur la densité de $X$ moins de max que de nombres de normales. Il peut y avoir plusieurs raison à cela, des moyennes très similaires, une proportion très faible d'espèce, des écart-types très élevés, tous ceci faisant confondre possiblement les normales entre elles. \\

Pour remédier à ce problème, et quant à la suite des choix des paramètres, même dans des cas d'hétérogénéité, nous allons introduire un partitionnement de l'échantillon par méthode des k-means.\\

La fonction $param\_kmeans$ va effectuer un partionnement par méthode des k-means sur le choix obtenu des moyennes (en les max de la densité de $X$). 
Cette méthode permettra de ressembler les valeurs les plus proches de ces centres ensembles, il sera ensuite facile et rapide de déterminer une approximation de la proportion de chaque espèce ainsi que l'écart-type de leur loi associée.

Cependant, comme indiqué juste au-dessus, il est possible que l'on ai un melange homogène entre les espèces, ceci peut amener à avoir moins de maximum qu'il y a d'espèces présentent et peut biaiser une bonne initialisation des moyennes. En voici une représentation du problème.

image.

Pour tenter de résoudre ceci, la fonction utilise un k-means sur les ces centres obtenus par maximum de la densité de $X$. Puis elle retient le cluster ayant le plus grand nombre de valeur et effectue un nouveau k-means sur ce sous échantillon, avec 2 centres initiaux, ces centres seront choisis automatiquement par la fonction $kmeans()$ de R parmi un choix de 25 initialisations. 

De cette manière on actualise le choix des moyennes, la fonction effectuera autant de répétion nécéssaire, jusqu'à avoir un nombre de moyennes égale au nombre d'espèces. 
En fin elle effectue un nouveau kmeans sur ces moyennes et relève les proportions associées aux clusters formés ainsi que leur écart-type.\\

Nous allons maintenant décrire quelques parties de code de la fonction $param\_kmeans$ : 
...
Pour ce faire on relève tous les maximums en dehors de l'échantillon, et on supprime un part un ceux les plus éloignés de bornes de l'échantillon. Si cela ne suffit pas, on supprime les moyennes les moins significatifs (les plus faible) jusqu'à avoir un nombres de maximums égale au nombre d'espèces.
...
Le code ayant peu d'intérêt à présenter, on laissera le lecteur le choix de le voir en Annexe.
